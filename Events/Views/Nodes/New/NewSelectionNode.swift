...
class NewSelectionNode: TableCell {
    ...
    func setNode(title: String, subtitle: String? = nil) {
        titleNode.attributedText = title.setAttributes(
            withFont: UIFont(name: "OpenSans-SemiBold", size: 17)!, textColor: .black, aligment: .left
        )
        
        subtitleNode.attributedText = (subtitle ?? "Select").setAttributes(
            withFont: UIFont(name: "OpenSans-Regular", size: 16)!,
            textColor: subtitle == nil ? Constants.text_placeholder_color : .black, aligment: .left
        )
    }
